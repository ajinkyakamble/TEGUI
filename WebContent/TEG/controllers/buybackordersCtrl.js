angular.module("buybackorders.controller", ['teg.factory'])
    .controller('buybackordersCtrl', function($scope, $rootScope, $sessionStorage, myConfig, Urlconfig, TegServices, LoginServices, $location, $mdpDatePicker, $sce) {
...
//--------------------------------------------------------------------------------------//
        ...
        $scope.selectedCardType.selected = $scope.cardTypeValues[0];
        ...
        TegServices.post(Urlconfig.getOrders, $scope.requestParams, {
            ...
            if ($scope.data != "") {
                ...
                angular.forEach($scope.data.orderList, function(value, key) {
                    ...
                    value['selectedcardType'] = "UK Visa MasterCard Debit";
                    ...
                    // }
                    // value.orderDetails['ordercharge'] = ((0.21 / 100) * total).toFixed(2);
                    ...
                    value.orderDetails['total'] = Number(Math.round(value.totalOrderAmount+'e2')+'e-2');
                        ...
                        value.orderDetails['ordercharge'] = Number(Math.round(((0.21 / 100) * value.orderDetails.total) +'e2')+'e-2');
        ...
        $scope.currrencyRangeFilter = function(rangeValue, currFrom, currTo) {
            ...
            currFrom = currFrom.replace(",", "");
            currTo = currTo.replace(",", "");
                    ...
                    angular.forEach($scope.data.orderList, function(value, key) {
                        ...
                        angular.forEach(value.orderDetails, function(value, key) {
                            ...
                            if (value.sterlingOrGBPAmount.toString().indexOf(".") == -1) {
                                ...
                                //value['sterlingOrGBPAmount'] = value.sterlingOrGBPAmount.toString().slice(0, (value.sterlingOrGBPAmount.toString().indexOf(".")) + 3);
                                value['sterlingOrGBPAmount'] = Number(Math.round(value.sterlingOrGBPAmount+'e2')+'e-2');
                            }
                            ...
                            if (value.amount.toString().indexOf(".") == -1) {
                                ...
                                //value['amount'] = value.amount.toString().slice(0, (value.amount.toString().indexOf(".")) + 3);
                                value['amount'] = Number(Math.round(value.amount+'e2')+'e-2');
                            }
                ...
                if (value.orderNo == orderno) {
                    switch (selectedCard) {
                        case 'UK Visa MasterCard Debit':
                            ...
                            value.orderDetails['ordercharge'] = cal.toFixed(2);
                            $scope.percentValue = '0.21%';
                            value.orderDetails['orderfinalCharge'] = (Number(gbpamount) + Number(value.orderDetails.ordercharge)).toFixed(2);
                            break;
                        case 'UK Credit Card':
                            if (gbpamount <= 300)
                                var cal = 3;
